...
public:
	...
	// find looks up the given key in the hash table and returns
	// the data value if it is found or else it returns 0, given that
	// the hash value has already been computed.
	void *find(unsigned long h, HashKey *key) {
	...
	void add(HashKey *key, void *data) {
		add(key->hash(), key, data);
	}

	// add adds a value to the hash table given that the
	// hash value has been pre-computed.
	void add(unsigned long h, HashKey *key, void *data) {
		Node *n = new Node();
		n->key = key;
		n->data = data;
		n->hash = h;
