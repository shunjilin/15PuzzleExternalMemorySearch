...
struct Tiles : public SearchDomain {
	...
	virtual int nops(SearchState *ss) {
		State *s = static_cast<State*>(ss);
		return optab[(int) s->blank].n;
	}

	virtual int nthop(SearchState *ss, int n) {
		State *s = static_cast<State*>(ss);
		return optab[(int) s->blank].ops[n];
	}

	struct Undo : public UndoToken { char h, blank; };

	virtual Edge apply(SearchState *ss, int newb) {
		State *s = static_cast<State*>(ss);
		Edge e(1, newb, s->blank);
		Undo *u = new Undo();
		u->h = s->h;
		u->blank = s->blank;
		e.as.undo = u;
	...
	virtual void undo(SearchState *ss, Edge &e) {
		State *s = static_cast<State*>(ss);
		Undo *u = static_cast<Undo*>(e.as.undo);
		s->h = u->h;
		s->tiles[(int) s->blank] = s->tiles[(int) u->blank];
		s->blank = u->blank;
		delete u;
	}
